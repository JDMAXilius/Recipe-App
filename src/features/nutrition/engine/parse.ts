// Port of backend/src/lib/nutrition/parseIngredient.js (B1.1) — ingredient
// line → structure. "2 1/2 cups plain flour" → { qty: 2.5, unit: "cup",
// item: "plain flour", grams: ~312, confidence }. Deterministic, no LLM.
// Grams are ESTIMATES (density fallbacks) and every unresolved qty/unit/
// density is flagged via confidence — honesty first.
//
// PORT, not a rewrite: behavior is pinned by the v1 parseIngredient suite.
// Comments explaining WHY a row exists carry over with the row.
import VERIFIED_PIECE from "./data/pieceWeights.json" with { type: "json" };
import CUP_WEIGHTS from "./data/cupWeights.json" with { type: "json" };
...
type PieceRow = { grams: number; unit?: string | null };
type CupRow = { gramsPerCup: number };
...
const VOLUME_ML: Record<string, number> = { cup: 240, tbsp: 15, tsp: 5, ml: 1, l: 1000, quart: 946, pint: 473 };
// small/approximate units → grams, rough by nature
const APPROX_G: Record<string, number> = { pinch: 0.4, dash: 0.6, drop: 0.05, knob: 15, handful: 30, splash: 10 };
...
const PIECE_G: [RegExp, RegExp, number, string?][] = [
  [/^clove$/, /garlic/i, 3],
...
function densityFor(item: string): { density: number; matched: boolean } {
...
function verifiedPiece(item: string | null | undefined, unit: string | null): PieceRow | null {
  ...
  let best: { name: string; row: PieceRow } | null = null;
  for (const [name, row] of Object.entries(VERIFIED_PIECE as Record<string, PieceRow>)) {
...
function cupWeightFor(item: string | null | undefined): CupRow | null {
  ...
  const sized = SIZE_QUALIFIER.test(t);
  const forms = sized ? singulars(t) : [...singulars(t), ...singulars(t.split(/\s+/).pop() || "")];
  let best: { name: string; row: CupRow } | null = null;
  for (const [name, row] of Object.entries(CUP_WEIGHTS as Record<string, CupRow>)) {
    const hit = forms.some((f) => f === name || (!sized && f.endsWith(" " + name)));
...
function gramsFor(qty: number | null, unit: string | null, item: string): { grams: number | null; level: string } {
...
export function expandPackSize(text: string): string {
  const m = text.match(
    new RegExp(
...
export type IngredientPair = { measure?: string | null; name?: string | null };

// Line → { qty, unit, item, grams, confidence, raw }.
// Accepts a string ("2 cups flour") or the app's { measure, name } pair.
export function parseIngredientLine(input: string | IngredientPair): ParsedLine {
  const raw =
    typeof input === "string"
      ? input
...
export function parseIngredients(list: (string | IngredientPair)[] | null | undefined): ParsedIngredients {
  const lines = (list || []).map(parseIngredientLine);
  const withGrams = lines.filter((l) => l.grams != null);
  const totalGrams = withGrams.reduce((sum, l) => sum + (l.grams as number), 0);
  ...
  const confidence: Confidence = lowShare > 0.25 ? "low" : lowShare + medShare > 0.25 ? "medium" : "high";
