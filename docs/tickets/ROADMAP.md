...
| SB-11 | `supabase/migrations/tests/rls-attacks.test.mjs` is stale: it seeds a `collab_lists` table and calls `admin_delete_user_data`, and neither exists any more, so it crashes after sign-up (2026-09-24). SB-6 was verified by hand instead: SQL impersonation as a stranger, the owner and anon on plan_entries, recipes, households and household_members, and every cross-user read returned 0. Rewrite the script against today's schema and make it clean up its own users. 8 old throwaway `otto-rls-*@example.com` users from July are still in auth (safe to delete). | Claude | P2 | todo |
