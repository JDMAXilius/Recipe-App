...
This **supersedes most of the blocked ticket work**: T1's frying curation, T2's *per-recipe*
audit half, T4, T5's remainder, T7's recompute, and the whole `recipeFacts.json` overlay all
collapse into "the canonical data is simply correct." One effort, at the source, one time.
(NOT absorbed: **T3** cooked-yield factors and **T2's weight-layer consolidation** — those
guard *user-imported* recipes, which this migration deliberately doesn't touch. Authoritative
list: "Relationship to NUTRITION_ACCURACY.md" at the bottom.)
...
- **Ingredient lists are facts** — not copyrightable (US). Instruction *prose* can carry thin
  ...
  them). **Images are copyrighted** → keep attribution + hotlink, or re-host the **existing
  files as-is** if the terms allow; never silently re-host, and never replace/regenerate
  (founder call — media is kept exactly as it is).
...
Read TheMealDB terms directly; confirm supporter status; decide image strategy (**re-host the
existing files vs keep hotlinking** — never replace/regenerate). Gate for everything below.
...
- each ingredient line → `{ original, key (an existing usdaTable key — Claude must pick from
  the shipped key list, never invent; null when nothing fits), grams (metric), cooked?,
  frying_medium?, note }` — the field is named **`key`** everywhere (silver shape = the schema
  of record; the canonicalizer emits it and the driver's zod schema validates it)
...
with Otto-original recipes (`provenance.source = "otto"`, plus promoted `otto-ai` generations
and `user-created` gems — see Phase 7), every one entering through the same
...
```jsonc
{
  ...
  "ingredients": [{
    ...
    "key": "beef",                        // canonical usdaTable key — the ontology join.
                                          // null = no key fits (surfaces in the run's
                                          // missing_ingredients report → Phase 3 top-up)
    "grams": 1000,
    "cooked": false, "frying_medium": false,
    "note": null                          // set when an amount was inferred (doubt survives)
  }],
  // The canonicalizer's run-level outputs `missing_ingredients[]` and `doubts[]` are NOT
  // stored per-recipe: they go to the run report + the ticket Log (missing keys feed
  // Phase 3; doubts feed the human review queue). Per-line doubt persists via `note`.
...
- **One JSONB `canonical` column** in `otto_recipes` holding the whole silver record (schema:
  `id, canonical jsonb, provenance jsonb, created_at, updated_at` — matches the kickoff
  ticket), not a normalized child table — the app only ever reads whole recipes; zero joins.
  Search-by-ingredient later = a GIN/expression index on `canonical->'ingredients'`, no
  schema change. Normalization can be added later without touching silver.
...
**APPROVED 2026-07-23** (founder directive — see `TERMINAL_TICKET_OTTO_RECIPES_KICKOFF.md`).
This is the AUTHORITATIVE supersession list: T6 stays (the auditor). T1-mechanism stays
(guards user imports). **T3 and T2's weight-layer consolidation stay live** (they guard
user-imported recipes, untouched by this migration). T1-data-curation, T2's per-recipe half,
T4, T5-remainder, and T7 are **absorbed** into Phases 2–5 above — closed-by-supersession; they
revive only if this migration is cancelled.
