...
| P4 Rich features | ☑ **DONE.** planner picker/swap/leftovers (`a2a11a87`) · Ask-Otto chat (`0f38f198`, verified live) · AI/nutrition/content edge fns DEPLOYED, all 3 keys set: generate-recipe chat+AI+**vision**, import-recipe URL, resolve-nutrition live-USDA (Atwater fix), content **v2 supporter** · client nutrition fallback (`62a97571`) · pref-aware pick + Tonight band (`92e30ae4`) · notifications + journal + vision import (`75ea5742`) · **native OAuth** (`09a13033`) verified live on device (Apple sheet + Google/FB browser; paid team `A6J6HGNWZK`) · auth REQUIRED, guest removed. Dev build rebuilt clean with all native modules. Remaining = device-pass runtime taps (camera capture, notif-permission prompt, real Apple/Google login) — code + backends verified |
