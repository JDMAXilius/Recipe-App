...
test("accepts a plain invite URL", () => {
  assert.equal(parseInviteToken(`https://ottosapp.com/hl/${TOKEN}`), TOKEN);
});

test("accepts the shapes messaging apps actually produce", () => {
  assert.equal(parseInviteToken(`https://ottosapp.com/hl/${TOKEN}/`), TOKEN);
  assert.equal(parseInviteToken(`https://ottosapp.com/hl/${TOKEN}?utm_source=whatsapp`), TOKEN);
  assert.equal(parseInviteToken(`Join our list https://ottosapp.com/hl/${TOKEN} — I'll grab milk`), TOKEN);
});
